...
# --- SortGame Class (Enhanced Mock) ---
class SortGame:
    """
    Initializes the game.
    'ranking' is the ground truth (index -> rank).
    'rel' is the (potentially noisy) relation matrix for dirtyCompare.
    """
    def __init__(self, n: int, ranking: List[int], rel: Optional[List[List[bool]]] = None):
        self.n = n
        ...
        if rel:
            ...
            # If no relation matrix is provided, create a "perfect" one from the ranking.
            self.rel = [[False for _ in range(n)] for _ in range(n)]
            for i in range(n):
                for j in range(n):
                    if i == j: continue
    ...
    def compare(self, i: int, j: int) -> bool:
        """The 'clean' comparison: uses ground truth ranking and counts."""
        self.comparison_count += 1
        return self.ranking[i] < self.ranking[j]

    def dirtyCompare(self, i: int, j: int) -> bool:
        """The 'dirty' comparison: uses the (potentially noisy) 'rel' matrix."""
        # This comparison is not counted, mimicking the C++ implementation.
        return self.rel[i][j] 
...
class DirtyCleanTree:
    ...
    def _rebuild_tree(self, u: Node):
        ...
        a: List[Optional[Node]] = [None] * n_size
        self._store_in_array(u, a, 0) # type: ignore

        new_root = self._build_balanced_from_array(a, 0, n_size) # type: ignore
    ...
    def freeze_insert(self, value: int, dirty: Callable[[int, int], bool], clean: Callable[[int, int], bool]) -> Node:
        ...
        if inserted_node is None:
            # This should not happen if root was handled in _bst_insert_custom
        ...
        if depth > h_alpha:
            ...
            # Find the scapegoat node 'p'
            while p and self.size(p) <= self.ALPHA * self.size(p.parent):
                p = p.parent
            # Rebuild from the scapegoat's parent
            if p and p.parent:
        ...
        # We find the nearest clean predecessor (st) and successor (ed) *in the whole tree*.
    ...
    def del_node(self, target_node: Node):
        ...
        if not target_node.left:
            ...
            # Recurse on the successor node. This is a common way to handle deletion.
            return self.del_node(successor) 
...
class DirtyClean4:
    ...
    def sort(self, game: SortGame) -> List[int]:
        ...
        for ai in shuffledA:
            ...
            # Reverting to the C++ logic structure for fidelity
...
if __name__ == '__main__':
    # This main block is for simple local testing of this file.
    # The mainDirtyClean.py file is the actual test harness.
    ...
    # Item 3 has rank 0
    # Item 4 has rank 2
    # True sorted order (by index): 3, 1, 4, 0, 2
    ...
    if len(ground_ranking) != N or sorted(ground_ranking) != list(range(N)):
        ...
        # Test with a "perfect" relation matrix (default SortGame behavior)
        ...
        print(f"--- Simple Local Test (Clean) ---")
